...
print("🌌 v1.6.2 — Stable Magnetic Energy Evolution (Fixed)")
...
mu0 = 4 * np.pi * 1e-7
eta = 0.0015          # slightly higher resistivity for stability
dt = 0.00035          # smaller timestep
steps = 500
max_v = 350 * 1000.0  # hard velocity cap

r_cyl = np.sqrt(X**2 + Y**2)
r_sph = np.sqrt(X**2 + Y**2 + Z**2 + 1e-8)

# ====================== FIELDS ======================
rho = 2.8e-24 * np.exp(-r_cyl / 13.0) * np.exp(-np.abs(Z)/5.0)
J_z = 1.1e18 * np.exp(-r_cyl**2 / (2*6.5**2))   # reduced current
...
for step in range(steps):
    ...
    # Velocity update with strong limiter
    vx += dt * Fx / (rho + 1e-30)
    vy += dt * Fy / (rho + 1e-30)
    vz += dt * Fz / (rho + 1e-30)
...
plt.suptitle('v1.6.2 Stable Magnetic Energy Evolution')
plt.tight_layout()
plt.show()
