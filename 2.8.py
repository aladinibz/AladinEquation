...
print("🌌 Plasma Cosmology v2.8 — Clean Stable CT with Proper Interpolation")
...
print("Starting v2.8 with fixed interpolation...\n")

start = time.time()

for step in range(steps):
    # Cell-centered B (proper interpolation)
    Bx_c = (Bx[:-1] + Bx[1:]) / 2
    By_c = (By[:,:-1] + By[:,1:]) / 2
    Bz_c = (Bz[:,:,:-1] + Bz[:,:,1:]) / 2

    # Edge EMFs using cell-centered fields (simplified but stable)
    Ex = -(vy * Bz_c - vz * By_c)
    Ey = -(vz * Bx_c - vx * Bz_c)
    Ez = -(vx * By_c - vy * Bx_c)
    ...
    # Lorentz force (using cell-centered B)
...
plt.suptitle('Plasma Cosmology v2.8 — Stable CT')
plt.tight_layout()
plt.show()
