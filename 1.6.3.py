...
print("🌌 v1.6.3 — Strongly Stabilized Magnetic Energy Evolution")

# ====================== GRID ======================
N = 80   # Start conservative, increase later
L = 45.0
dx = L / N
...
mu0 = 4 * np.pi * 1e-7
eta = 0.0020          # higher resistivity for stability
dt = 0.00025          # much smaller timestep
steps = 600
max_v = 300 * 1000.0
...
# ====================== FIELDS ======================
rho = 3.0e-24 * np.exp(-r_cyl / 13.0) * np.exp(-np.abs(Z)/5.0)
J_z = 8.0e17 * np.exp(-r_cyl**2 / (2*7.0**2))   # further reduced

v0 = 230 * 1000.0
v_theta = v0 * (1 - np.exp(-r_cyl / 7.0))
...
for step in range(steps):
    ...
    # Strong velocity limiter BEFORE update
    ax = Fx / (rho + 1e-30)
    ay = Fy / (rho + 1e-30)
    az = Fz / (rho + 1e-30)
    ...
    mask = v_tot > max_v
    if np.any(mask):
        vx[mask] = vx[mask] * (max_v / (v_tot[mask] + 1e-30))
        vy[mask] = vy[mask] * (max_v / (v_tot[mask] + 1e-30))
        vz[mask] = vz[mask] * (max_v / (v_tot[mask] + 1e-30))
...
plt.plot(time_steps, np.array(e_mag_list)/total, 'magenta', label='Mag')
plt.plot(time_steps, np.array(e_kin_list)/total, 'cyan', label='Kin')
...
plt.suptitle('v1.6.3 Stabilized Magnetic Energy Evolution')
plt.tight_layout()
plt.show()
